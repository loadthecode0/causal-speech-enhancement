...
for epoch in range(num_epochs):
    ...
    for batch_idx, (clean_waveform, noisy_waveform) in enumerate(train_loader_tqdm):
        ...
        # Teacher's predictions (softened logits for distillation)
        with torch.no_grad():
    ...
    checkpoint_path = os.path.join(model_dir, f"conv_tasnet_causal_transfer_kld_epoch_{epoch + 1}.pth")
    ...
    # Save best model
    best_model_path = os.path.join(model_dir, f"conv_tasnet_causal_transfer_kld_best_model.pth")
...
training_curve_path = os.path.join(stats_dir, "conv_tasnet_causal_transfer_kld_training_curve.png")
plt.figure(figsize=(10, 6))
plt.plot(range(1, num_epochs + 1), train_losses, label="Train Loss")
plt.plot(range(1, num_epochs + 1), val_losses, label="Validation Loss")
plt.xlabel("Epoch")
plt.ylabel("Loss")
plt.title("Training and Validation Loss Curve (Transfer Learning by KLD)")
plt.legend()
plt.grid(True)
